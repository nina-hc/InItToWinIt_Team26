...
/**
 * BuildCity handles the logic for upgrading an existing settlement into a city.
 * BuildCity is responsible to check if the player can afford a city, choose a
 * valid settlement place to upgrade, validate that the settlement belongs to
...
public class BuildCity extends Build {
	...
	/**
	 * Checks whether the player is allowed to buy a city The conditions are that
	 * the player must have enough resources and the player must still have cities
	 * left in inventory
	 *
	...
	/**
	 * Selects which settlement will be upgraded If the player has no settlements,
	 * they can't upgrade Otherwise, the program will randomly pick one of the
	...
	/**
	 * Validates that the chosen settlement can be upgraded To do this
	 * validatePlacement must check that the settlement belongs to the player.
	 *
...
//
	 ...
	 * Deducts city resources 2. Creates a new city 3. Places a city on the node 4.
